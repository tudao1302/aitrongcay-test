...
<?php if (in_array($slug, ['portal/ban-be', 'portal/hang-xom'], true)) : ?>
    ...
    <div class="eco-friends-grid" style="margin-top:24px">
      <?php
        $leaderboard_query = new WP_User_Query([
            'meta_key'   => '_aitrongcay_eco_points',
            'orderby'    => 'meta_value_num',
            'order'      => 'DESC',
            'number'     => 10,
        ]);
        $top_users = $leaderboard_query->get_results();
      ?>
      <?php if (!empty($top_users)): ?>
      <section class="eco-friends-panel" style="background: linear-gradient(145deg, rgba(20,24,20,0.95), rgba(12,14,12,0.98)); border: 1px solid rgba(111,219,168,0.2); margin-bottom:24px;">
        <h4 style="color:#6fdba8; display:flex; align-items:center; gap:8px;">🏆 Bảng xếp hạng Nông dân 4.0</h4>
        <div class="eco-friend-sub" style="margin:-4px 0 18px">Top 10 người dùng có điểm sinh thái (Eco Points) cao nhất hệ thống. Cày điểm ngay để leo rank!</div>
        
        <div style="display:flex; flex-direction:column; gap:12px;">
            <?php 
            $rank = 1;
            foreach ($top_users as $top_user): 
                $points = (int) get_user_meta($top_user->ID, '_aitrongcay_eco_points', true);

                $medal = '';
                $bg_color = 'rgba(255,255,255,0.03)';
                $border_color = 'rgba(255,255,255,0.05)';
                if ($rank === 1) { $medal = '🥇'; $bg_color = 'rgba(255,215,0,0.1)'; $border_color = 'rgba(255,215,0,0.3)'; }
                elseif ($rank === 2) { $medal = '🥈'; $bg_color = 'rgba(192,192,192,0.1)'; $border_color = 'rgba(192,192,192,0.3)'; }
                elseif ($rank === 3) { $medal = '🥉'; $bg_color = 'rgba(205,127,50,0.1)'; $border_color = 'rgba(205,127,50,0.3)'; }
                else { $medal = '<span style="color:#6fdba8;font-weight:bold;">#'.$rank.'</span>'; }
                
                // Get badge for the top user
                $specialty_badges = [
                    0 => ['icon' => '🌹', 'label' => 'Chuyên gia hoa hồng', 'color' => '#f43f5e'],
                    1 => ['icon' => '🥬', 'label' => 'Vua rau sạch', 'color' => '#22c55e'],
                    2 => ['icon' => '🐛', 'label' => 'Thợ diệt sâu', 'color' => '#eab308'],
                    3 => ['icon' => '🪴', 'label' => 'Người chơi hệ lá', 'color' => '#14b8a6']
                ];
                $user_specialty = get_user_meta($top_user->ID, '_aitrongcay_specialty_badge', true);
                if ($user_specialty === '' || !isset($specialty_badges[$user_specialty])) {
                    $user_specialty = $top_user->ID % 4;
                }
                $s_badge = $specialty_badges[$user_specialty];
                $specialty_html = '<span style="display:inline-flex; align-items:center; gap:4px; padding:2px 6px; border-radius:8px; font-size:10px; color:'.$s_badge['color'].'; border:1px solid '.$s_badge['color'].'40"><span aria-hidden="true">'.$s_badge['icon'].'</span> '.$s_badge['label'].'</span>';
            ?>
            <div style="display:flex; align-items:center; justify-content:space-between; background:<?php echo $bg_color; ?>; border-radius:16px; padding:12px 16px; border:1px solid <?php echo $border_color; ?>;">
                <div style="display:flex; align-items:center; gap:12px;">
                    <div style="font-size:24px; min-width:32px; text-align:center;"><?php echo $medal; ?></div>
                    <div style="width:40px; height:40px; border-radius:12px; background:rgba(111,219,168,.12); display:grid; place-items:center; font-size:20px;">🌿</div>
                    <div>
                        <div style="font-weight:700; font-size:16px; color:#fff; display:flex; align-items:center; gap:8px;">
                            <?php echo esc_html($top_user->display_name ?: $top_user->user_login); ?>
                            <?php if ($top_user->ID === $current_user->ID): ?>
                                <span style="background:#6fdba8; color:#062013; font-size:10px; padding:2px 6px; border-radius:8px; font-weight:800; text-transform:uppercase;">Bạn</span>
                            <?php endif; ?>
                        </div>
                        <div style="display:flex; align-items:center; gap:6px; margin-top:4px;">
                            <div style="font-size:12px; color:rgba(255,255,255,0.5);">Nông dân hệ sinh thái</div>
                            <?php echo $specialty_html; ?>
                        </div>
                    </div>
                </div>
                <div style="text-align:right;">
                    <div style="font-weight:800; font-size:18px; color:#6fdba8;"><?php echo number_format($points); ?></div>
                    <div style="font-size:11px; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:0.5px;">Eco Points</div>
                </div>
            </div>
            <?php $rank++; endforeach; ?>
        </div>
      ...
      <section class="eco-friends-panel"><h4>Hàng xóm hiện tại</h4><div class="eco-friend-sub" style="margin:-4px 0 18px"><?php echo esc_html($friend_count > 0 ? 'Hiện tại có ' . $friend_count . ' hàng xóm' : 'Hiện tại chưa có hàng xóm nào'); ?></div><?php if ($friends) : ?><div class="eco-friends-stack"><?php $my_primary_garden_key = function_exists('aitrongcay_primary_garden_key_for_user') ? aitrongcay_primary_garden_key_for_user($current_user) : (function_exists('aitrongcay_current_garden_key') ? aitrongcay_current_garden_key($current_user) : ''); $my_garden_members = $my_primary_garden_key !== '' ? aitrongcay_get_garden_members($my_primary_garden_key) : []; $friend_membership_map = []; foreach ($my_garden_members as $member) { $member_user_id = (int) ($member['user_id'] ?? 0); if ($member_user_id > 0) { $friend_membership_map[$member_user_id] = $member; } } $today_ymd = current_time('Ymd'); foreach ($friends as $friendship) : $friend_id = (int) (($friendship['requester_user_id'] == $current_user->ID) ? $friendship['addressee_user_id'] : $friendship['requester_user_id']); $friend = get_user_by('id', $friend_id); $friend_garden_key = $friend instanceof WP_User ? aitrongcay_preferred_garden_key_for_user($friend) : ''; $friend_garden_url = $friend_garden_key !== '' ? add_query_arg('garden', rawurlencode($friend_garden_key), home_url('/portal/dashboard-2/')) : home_url('/portal/dashboard-2/'); $friend_phone = $friend instanceof WP_User ? aitrongcay_user_zalo_phone((int) $friend->ID) : ''; $friend_zalo_url = $friend_phone !== '' ? 'https://zalo.me/' . rawurlencode($friend_phone) : ''; $membership = $friend_membership_map[$friend_id] ?? null; $friend_role = (string) ($membership['role'] ?? 'viewer'); $membership_id = (int) ($membership['id'] ?? 0); $share_label = $friend_role === 'co_owner' ? 'Đã share vườn' : 'Share vườn'; $share_url = aitrongcay_friend_toggle_share_url($friend_id, $membership_id, $friend_role, $my_primary_garden_key); $remove_url = aitrongcay_remove_friend_url($friend_id); $helpful_count = (int) get_user_meta($friend_id, '_aitrongcay_helpful_water_count', true); $badge_html = ''; if ($helpful_count >= 50) { $badge_html = '<span style="background:linear-gradient(135deg,#ffd700,#f59e0b);color:#fff;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700;margin-left:8px;box-shadow:0 2px 4px rgba(245,158,11,0.3)">🏆 ' . $helpful_count . ' lần giúp đỡ</span>'; } elseif ($helpful_count >= 10) { $badge_html = '<span style="background:linear-gradient(135deg,#34d399,#10b981);color:#fff;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:700;margin-left:8px">🏅 ' . $helpful_count . ' lần giúp đỡ</span>'; } elseif ($helpful_count > 0) { $badge_html = '<span style="background:rgba(255,255,255,0.1);color:#e3e3de;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600;margin-left:8px">💧 ' . $helpful_count . ' lần giúp đỡ</span>'; } $specialty_badges = [ 0 => ['icon' => '🌹', 'label' => 'Chuyên gia hoa hồng', 'color' => '#f43f5e'], 1 => ['icon' => '🥬', 'label' => 'Vua rau sạch', 'color' => '#22c55e'], 2 => ['icon' => '🐛', 'label' => 'Thợ diệt sâu', 'color' => '#eab308'], 3 => ['icon' => '🪴', 'label' => 'Người chơi hệ lá', 'color' => '#14b8a6'] ]; $friend_specialty = get_user_meta($friend_id, '_aitrongcay_specialty_badge', true); if ($friend_specialty === '' || !isset($specialty_badges[$friend_specialty])) { $friend_specialty = $friend_id % 4; } $s_badge = $specialty_badges[$friend_specialty]; $specialty_html = '<div style="display:inline-flex; align-items:center; gap:4px; background:rgba(255,255,255,0.05); padding:3px 8px; border-radius:12px; margin-top:6px; font-size:11px; color:'.$s_badge['color'].'; border:1px solid '.$s_badge['color'].'40"><span aria-hidden="true">'.$s_badge['icon'].'</span> '.$s_badge['label'].'</div>'; $has_watered_friend = get_user_meta($current_user->ID, "_aitrongcay_watered_{$friend_id}_{$today_ymd}", true); ?><article class="eco-friend-card"><div class="eco-friend-avatar">🌿</div><div><div class="eco-friend-name"><?php echo esc_html($friend instanceof WP_User ? ($friend->display_name ?: $friend->user_login) : 'Người dùng'); ?><?php echo $badge_html; ?></div><div class="eco-friend-sub" style="margin-top:2px;">Hàng xóm trong hệ sinh thái vườn số</div><?php echo $specialty_html; ?></div><div class="eco-friend-actions"><a class="eco-friend-action" href="<?php echo esc_url($friend_garden_url); ?>">Xem vườn</a><button type="button" class="eco-friend-primary aitr-water-friend" data-friend-id="<?php echo esc_attr((string)$friend_id); ?>" <?php if ($has_watered_friend) echo 'disabled style="background:#10b981;color:#fff"'; else echo 'style="background:linear-gradient(135deg, #3b82f6, #60a5fa);color:#fff"'; ?>><?php echo $has_watered_friend ? '✅ Đã tưới xong' : '💦 Tưới nước hộ'; ?></button><?php if ($friend_zalo_url !== '') : ?><a class="eco-friend-zalo" href="<?php echo esc_url($friend_zalo_url); ?>" target="_blank" rel="noopener" aria-label="Nhắn Zalo"><span aria-hidden="true"><svg viewBox="0 0 24 24" width="16" height="16" fill="none"><rect x="3" y="3" width="18" height="18" rx="6" fill="#fff"></rect><path d="M7.5 8.2h9l-6.05 7.6h5.95l-9 0 6.05-7.6H7.5Z" fill="#0068FF"></path></svg></span><span>Zalo</span></a><?php else : ?><button class="eco-friend-action" type="button" disabled>Chưa có Zalo</button><?php endif; ?><a class="eco-friend-primary<?php echo $friend_role === 'co_owner' ? ' is-co-owner' : ''; ?>" href="<?php echo esc_url($share_url); ?>"><?php echo esc_html($share_label); ?></a><a class="eco-friend-danger" href="<?php echo esc_url($remove_url); ?>" aria-label="Hủy kết nối hàng xóm"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg></a></div></article><?php endforeach; ?></div><?php else : ?><div class="notice">Anh chưa có hàng xóm nào. Anh có thể tìm và kết nối bằng ô tìm kiếm phía trên.</div><?php endif; ?></section>
